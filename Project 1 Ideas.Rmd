...
First some notes on the data. The data on the search index of certain prices is taken from Google trends which collects the search queries of people within a specific region (here: United States of America). This data is aggregated on a monthly basis and normalized with a range from zero to 100. Already filtered out are duplicate searches in the sense that the same user made the same search multiple times within a short time-frame. This way we exclude the users which have already invested and constantly check the prices to look how their investment is doing. Data points are divided by total searches for the month and region to represent the relative popularity, i.e.\ no over-weighting of regions with more people than others which would given the same search behavior lead to differing popularities otherwise. 

The data on gold prices comes form the London Bullion Market Association Gold Price and the Federal Reserve Bank of St. Louis, it is measured in USD per troy ounce at 3:00pm. Aggregation is done via prices at the end of each month and not seasonally adjusted.
...
```
As the test rejects, given the data we cannot say that the data is not stationary. 
```{r 6}
